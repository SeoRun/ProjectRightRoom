...
<body>
			...
			<div class="right-box" id="hotel-info">
				...
				<div class="button">
					<button type="submit" class="reserve-button" id="reserve"
						value="결제" onclick="payment()">결제</button>
				</div>
			</div>
		</div>
	</form>
	...
	<script>
            ...
            const tableOptions = document.querySelectorAll('.box'); // 테이블 옵션 요소들을 선택합니다.
            const selectedTableArrangementInput = document.getElementById('selected-table-arrangement'); // 숨겨진 input 엘리먼트를 선택합니다.

            tableOptions.forEach(option => {
                option.addEventListener('click', () => {
                    const isClicked = option.classList.contains('clicked');

                    // 클릭한 박스를 다시 클릭하면 클릭이 해제됩니다.
                    if (isClicked) {
                        option.classList.remove('clicked');
                        option.style.backgroundColor = 'white';
                        option.style.color = 'black';
                        selectedTableArrangementInput.value = ''; // 클릭이 해제되면 값을 지웁니다.
                    } else {
                        // 클릭한 박스에 클릭 스타일 적용
