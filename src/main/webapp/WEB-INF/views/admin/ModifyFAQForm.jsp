...
<jsp:include page="../include/header.jsp" />
	...
	<form action="<c:url value='/admin/modifyFAQConfirm' />" name="ModifyFAQForm" method="post" enctype="multipart/form-data">
		<div class="container">
			<h2>게시글을 작성해 주세요.</h2>
			...
			<div class="input-box">
				<input type="text" id="title" name="faq_u_title" value="${userFAQVO.faq_u_title}" placeholder="제목을 입력해 주세요.">
			</div>
	
			<div class="input-box">
				<textarea id="content" rows="10" name="faq_u_content" placeholder="내용을 입력해 주세요.">${userFAQVO.faq_u_content}</textarea>
			</div>
	
			<div class="input-box">
				<div class="file-input-container">
					<input type="file" id="image" name="file" value="${userFAQVO.faq_u_image}" accept="image/*">
				</div>
			</div>
			<div class="buttons">
				<input type="button" id="delete" value="취소" onclick="goBack()">
				<input type="button" id="modify" value="수정" onclick="modifyFAQ()">
			</div>
			<input type="hidden" name="faq_u_no" value="${userFAQVO.faq_u_no}">
		</div>
	</form>
	...
	<script>
		...
		function modifyFAQ() {
			console.log('modifyFAQ() CALLED!!');
			
			let form = document.ModifyFAQForm;
